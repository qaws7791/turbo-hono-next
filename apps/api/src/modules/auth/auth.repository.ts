  ...
  return tryPromise(async () => {
    ...
    if (!user) {
      return Promise.reject(
        new ApiError(500, "USER_CREATE_FAILED", "사용자 생성에 실패했습니다."),
      );
    }
...
export function insertAuthSession(input: {
  ...
  return tryPromise(async () => {
    ...
    if (!session) {
      return Promise.reject(
        new ApiError(500, "SESSION_CREATE_FAILED", "세션 생성에 실패했습니다."),
      );
    }
...
export function updateSessionExpiryAndMetadata(input: {
...
export function findAuthAccountByProviderAccountId(input: {
...
export function insertAuthAccount(input: {
  readonly userId: string;
  readonly provider: typeof authAccounts.$inferSelect.provider;
  readonly providerAccountId: string;
...
export function updateAuthAccountScopes(input: {
...
export function insertMagicLinkToken(input: {
  readonly email: string;
...
export function findMagicLinkTokenByHash(
  tokenHash: string,
...
export function sendLoginLinkEmail(input: {
  readonly email: string;
...
export function exchangeGoogleOAuthCode(input: {
  ...
  return tryPromise(async () => {
    ...
    if (!res.ok) {
      return Promise.reject(
        new ApiError(502, "GOOGLE_TOKEN_EXCHANGE_FAILED", "토큰 교환 실패", {
          status: res.status,
          body: json,
        }),
      );
...
export function fetchGoogleUserInfo(
  accessToken: string,
  ...
  return tryPromise(async () => {
    ...
    if (!res.ok) {
      return Promise.reject(
        new ApiError(502, "GOOGLE_USERINFO_FAILED", "사용자 정보 조회 실패", {
          status: res.status,
          body: json,
        }),
      );
