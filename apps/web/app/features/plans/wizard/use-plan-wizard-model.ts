...
import type { PlanWizardModel, PlanWizardStep, PlanWizardValues } from "./types";

import { PlanGoalSchema, PlanLevelSchema } from "~/mock/schemas";
...
export function usePlanWizardModel(input: {
  ...
  const toggleDocument = React.useCallback(
    (documentId: string) => {
      setValues((prev) => {
        if (prev.selectedDocumentIds.includes(documentId)) {
          return {
            ...prev,
            selectedDocumentIds: prev.selectedDocumentIds.filter((id) => id !== documentId),
          };
        }
  ...
  const setDurationUnit = React.useCallback((value: "days" | "weeks" | "months") => {
