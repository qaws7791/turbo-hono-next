...
export function LandingView({ isAuthenticated }: { isAuthenticated: boolean }) {
  const primaryHref = isAuthenticated ? "/home" : "/login";

  return (
    <div className="bg-background text-foreground min-h-svh">
      <LandingHeader primaryHref={primaryHref} />
      <main>
        ...
        <LandingHowItWorks />
        <LandingPrinciples />
        <LandingPricing primaryHref={primaryHref} />
        <LandingFAQ />
        <LandingCTA primaryHref={primaryHref} />
      </main>
      <LandingFooter />
    </div>
  );
...
function LandingHeader({ primaryHref }: { primaryHref: string }) {
  return (
...
function LandingHero({ primaryHref }: { primaryHref: string }) {
  return (
...
function LandingPricing({ primaryHref }: { primaryHref: string }) {
  const tiers = [
    {
      title: "무료",
      price: "₩0",
...
function LandingCTA({ primaryHref }: { primaryHref: string }) {
  return (
    <Section
      title="지금 시작하세요"
      subtitle="무료입니다."
