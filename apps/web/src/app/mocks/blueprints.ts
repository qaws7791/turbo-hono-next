...
function createReviewSessionBlueprint(input: {
  ...
  const steps: Array<SessionStep> = [
    // === 1. 세션 인트로 ===
    {
      ...
      learningGoals: [
        "이전에 학습한 핵심 개념 빠르게 복기",
        "기억이 흐려진 부분 재확인",
        "실수하기 쉬운 포인트 점검",
      ],
      questionsToCover: [
        "함수형 업데이터를 언제 사용해야 할까요?",
        "React 18의 자동 배칭이란 무엇인가요?",
        "stale closure 문제를 어떻게 방지할 수 있나요?",
      ],
      prerequisites: ["React 기초", "useState Hook"],
      estimatedSeconds: 30,
      intent: "INTRO",
    },
    ...
    // === 3. 이해도 체크: 플래시카드 ===
    {
      ...
      type: "FLASHCARD",
      front:
        "React에서 상태를 업데이트할 때 '이전 값'을 기반으로 해야 한다면 어떤 패턴을 사용해야 할까요?",
      back: "함수형 업데이터 패턴\n\nsetState(prev => prev + 1)\n\n이 방식은 React의 배치 업데이트에서도 항상 최신 상태를 참조하므로 안전합니다.",
      estimatedSeconds: 45,
      intent: "RETRIEVAL",
    },
    ...
    // === 4. 이해도 체크: 4지선다 ===
    {
      id: "check-1",
      type: "CHECK",
      question: "다음 중 stale closure 문제가 발생할 수 있는 코드는?",
      options: [
        "setCount(prev => prev + 1)",
        "setCount(count + 1) // 이벤트 핸들러 내",
        "useReducer의 dispatch 호출",
        "setState({ value: newValue })",
      ],
      answerIndex: 1,
      explanation:
        "이벤트 핸들러 내에서 count + 1처럼 직접 값을 참조하면, 클로저가 생성된 시점의 count 값을 캡처하게 됩니다.",
      estimatedSeconds: 45,
      intent: "RETRIEVAL",
    },
    ...
    // === 5. 이해도 체크: 스피드 O/X ===
    {
      ...
      type: "SPEED_OX",
      statement:
        "React 18에서는 setTimeout 내의 여러 setState도 자동으로 배칭됩니다.",
      isTrue: true,
      explanation:
        "맞습니다! React 18부터는 이벤트 핸들러뿐만 아니라 setTimeout, Promise, fetch 콜백 등 모든 컨텍스트에서 자동 배칭이 적용됩니다.",
      estimatedSeconds: 30,
      intent: "RETRIEVAL",
    },
    ...
    // === 6. 이해도 체크: Cloze ===
    {
      ...
      type: "CLOZE",
      sentence:
        "함수형 업데이터를 사용하여 안전하게 상태를 증가시키려면 setCount({{blank}} => {{blank}} + 1)로 작성합니다.",
      blankId: "prev",
      options: ["prev", "count", "state", "value"],
      answerIndex: 0,
      explanation:
        "prev는 이전 상태 값을 나타내는 관례적인 이름입니다. 어떤 이름도 가능하지만, prev가 가장 널리 사용됩니다.",
      estimatedSeconds: 30,
      intent: "RETRIEVAL",
    },
    ...
    // === 7. 적용 활동 ===
    {
      ...
      type: "APPLICATION",
      scenario:
        "당신은 쇼핑몰의 장바구니 기능을 개발하고 있습니다. 사용자가 '수량 +1' 버튼을 빠르게 여러 번 클릭할 때도 정확하게 수량이 증가해야 합니다.",
      question: "다음 중 올바른 구현 방법은?",
      options: [
        "setQuantity(quantity + 1)을 사용한다",
        "setQuantity(prev => prev + 1)을 사용한다",
        "클릭마다 setTimeout을 사용하여 지연시킨다",
        "useRef를 사용하여 수량을 저장한다",
      ],
      correctIndex: 1,
      feedback:
        "정답입니다! 함수형 업데이터를 사용하면 빠른 연속 클릭에서도 각 업데이트가 올바르게 이전 값을 기반으로 계산됩니다.",
      estimatedSeconds: 60,
      intent: "PRACTICE",
    },
    ...
    // === 8. 세션 요약 ===
    {
      ...
      celebrationEmoji: "🎉",
      encouragement: "복습을 완료했어요! 반복 학습이 장기 기억의 핵심입니다.",
      completedActivities: [
        "플래시카드 1개",
        "4지선다 퀴즈 1개",
        "O/X 퀴즈 1개",
      ...
      keyTakeaways: [
        "함수형 업데이터: prev => newValue 패턴으로 stale state 방지",
        "React 18의 자동 배칭은 모든 컨텍스트에서 작동",
        "이전 값이 필요하면 반드시 함수형 업데이터 사용",
      ],
      nextSessionPreview: input.nextSessionTitle
        ? {
            title: input.nextSessionTitle,
            description: "다음 세션에서 더 심화된 패턴을 학습합니다.",
          }
        : undefined,
      estimatedSeconds: 30,
      intent: "WRAPUP",
