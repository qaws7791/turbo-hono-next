...
import { formatDateTime } from "@/features/learning-plan/model/date";
...
export function OverviewTab({ detail }: OverviewTabProps) {
  const dueDateLabel = formatLearningTaskDueDate(detail.dueDate);
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Card className="space-y-4 p-4">
          ...
          <div className="space-y-2 text-sm">
            <div>
              <span className="text-muted-foreground">학습 계획</span>
              <p className="font-medium text-foreground">
                {detail.learningPlan.title}
              </p>
            </div>
            <div>
              <span className="text-muted-foreground">목표</span>
              <p className="font-medium text-foreground">
                {detail.learningModule.title}
              </p>
              {detail.learningModule.description && (
          ...
          <div className="space-y-1 rounded-md bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
            <div>목표 순서: {detail.learningModule.order}</div>
