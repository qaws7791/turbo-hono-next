...
import { useLearningTaskDetail } from "@/features/learning-plan/hooks/use-learning-task-detail";
...
const LEARNING_TASK_TAB_VALUES = ["overview", "ai-note", "ai-quiz"] as const;
type LearningTaskTab = (typeof LEARNING_TASK_TAB_VALUES)[number];
const learningTaskTabParser = parseAsStringLiteral(
  LEARNING_TASK_TAB_VALUES,
...
function RouteComponent() {
      ...
      <AppPageLayout>
        <div className="flex items-center justify-center min-h-96">
          <div className="space-y-4 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              {learningTask.error?.message ||
                "존재하지 않는 세부 목표이거나 접근 권한이 없습니다."}
            </p>
            <Link
              to="/app/learning-plans/$learningPlanId"
  ...
  const detail = learningTask.data.data;
  return (
    <AppPageLayout>
      <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
            <div className="flex items-center gap-1">
              <Icon
                ...
                className="size-3.5"
              />
              <span>{detail.learningPlan.title}</span>
            </div>
            <Icon
            ...
            <div className="flex items-center gap-1">
              <Icon
                ...
                className="size-3.5"
              />
              <span>{detail.learningModule.title}</span>
            </div>
          </div>
        ...
        <Tabs
          ...
          onSelectionChange={(key) => {
            const nextTab = String(key) as LearningTaskTab;
