...
export const GoalActivityQuerySchema = z
  .object({
...
const GoalActivityBaseSchema = z.object({
  roadmapId: z.string().length(16).openapi({
    description: "로드맵 공개 ID (16자 NanoID)",
    example: "abcdEfghijklmnop",
  }),
  roadmapTitle: z.string().min(1).openapi({
    description: "로드맵 제목",
    example: "프론트엔드 전환 로드맵",
  }),
  goalId: z.string().uuid().openapi({
    description: "목표 공개 ID (UUID)",
    example: "30aa53ce-93e7-4f90-9f69-6c4de2fb1c5d",
  }),
  goalTitle: z.string().min(1).openapi({
    description: "목표 제목",
    example: "React 기본기 다지기",
  }),
  subGoalId: z.string().uuid().openapi({
    description: "세부 목표 공개 ID (UUID)",
    example: "f28c26fa-40f8-4f56-9fba-9b8f3d3e2ea7",
  }),
  subGoalTitle: z.string().min(1).openapi({
    description: "세부 목표 제목",
    example: "Hooks 패턴 정리하기",
  }),
});

const GoalActivityDueSchema = GoalActivityBaseSchema.extend({
...
const GoalActivityCompletedSchema = GoalActivityBaseSchema.extend({
...
export const GoalActivityDaySchema = z.object({
  date: isoDateString.openapi({
    description: "활동이 기록된 날짜 (YYYY-MM-DD)",
    example: "2024-05-12",
  }),
  due: z.array(GoalActivityDueSchema).openapi({
    description: "해당 날짜에 마감일이 도래한 세부 목표",
  }),
  completed: z.array(GoalActivityCompletedSchema).openapi({
    description: "해당 날짜에 완료된 세부 목표",
  }),
});

export const GoalActivityResponseSchema = z.object({
  range: z
    .object({
  ...
  items: z.array(GoalActivityDaySchema).openapi({
