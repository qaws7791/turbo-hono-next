...
import type { AuthLoggerPort } from "../../api/ports";
  ...
  return {
    ...
    insertAuthAccount(input: {
      ...
      return tryPromise(async () => {
        ...
        if (!account) {
          throw coreError({
            code: "AUTH_ACCOUNT_CREATE_FAILED",
            message: "OAuth 계정 생성에 실패했습니다.",
          });
        }
        ...
        try {
          ...
          if (e instanceof OAuth2RequestError) {
            throw coreError({
              ...
              message: `토큰 교환 실패: ${e.code}`,
              details: { code: e.code, description: e.description },
              cause: e,
            });
          }
      ...
      return tryPromise(async () => {
        ...
        if (!parsed.success) {
          throw coreError({
            ...
            message: "사용자 정보 응답 형식이 올바르지 않습니다.",
            details: {
        ...
        try {
          ...
          if (!payload) {
            throw coreError({
              ...
              message: "ID 토큰 페이로드가 없습니다.",
            });
          }

          if (!payload.iss) {
            throw coreError({
              code: "GOOGLE_ID_TOKEN_INVALID_ISSUER",
              message: "잘못된 발급자입니다.",
            });
          }
          ...
          if (!payload.sub || !payload.email) {
            throw coreError({
              code: "GOOGLE_ID_TOKEN_MISSING_CLAIMS",
              message: "ID 토큰에 필수 클레임이 없습니다.",
            });
          }
