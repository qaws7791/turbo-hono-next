...
import type { MaterialProcessingQueuePort } from "../../api/queue.types";
...
import type { R2StoragePort } from "../../api/ports";
  ...
  return function enqueueMaterialProcessing(
    ...
    > => {
      try {
        ...
        // 5. Queue에 작업 등록
        const jobResult = await deps.materialProcessingQueue.add(
          "process-material",
          {
            userId,
