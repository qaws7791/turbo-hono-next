...
Materials는 학습 자료를 업로드하고 관리하는 공간입니다. 자료 업로드 시 AI가 자동으로 문서를 분석합니다.
...
| **텍스트 입력** | 노트 직접 붙여넣기 (내부적으로 텍스트 파일로 변환되어 업로드됨) |
...
2. **난이도 매핑** → 선행지식 자동 연결
3. **Plan 생성 대기** → 경로는 자동 생성 안 됨, 사용자가 명시적으로 Plan 생성 필요
...
![Materials Wireframe List](assets/documents-list.png)
...
- **파일**: 드롭존 + 파일 선택 버튼
- **텍스트**: 텍스트 에디터 (입력된 텍스트는 클라이언트에서 `.txt` 파일로 생성하여 공용 파일 업로드 API를 통해 업로드됨)
...
- 빈 상태 일러스트 및 텍스트 설명
- "자료 업로드" 버튼
  ![Materials Wireframe Empty](assets/documents-empty.png)
